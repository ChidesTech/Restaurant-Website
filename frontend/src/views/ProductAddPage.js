...
props.history.push("/food-list");
  ...
  {loading ? (
          ...
          /> 
            ...
            className="form">
               <div style={{marginBottom:"-3rem", textAlign:"center"}} className=""><h2>Food Details</h2></div>
          <div >
...
{categories.length>0 ? categories.map(category=>(<option style={{padding:"1rem", fontSize:"1.6rem"}}  value={category.name}>{category.name}</option>)
 ...
 {/* <input type="text" id="category" required placeholder="Enter Food Category" value={category} onChange={(e)=> setCategory(e.target.value)}/> */}
          ...
          <div style={{display:"flex",justifyContent:"space-between" ,flexDirection:"row-reverse", marginTop:"1rem"}} >
              ...
              <button onClick={()=>deleteHandler(productId)} style={{width:"40%", backgroundColor:"transparent", borderColor:"transparent", color:"red", boxShadow:"1px 1px 1px 1px red"}} type="button" className="btn">Cancel</button>
